...
Tags: surface, briefs, verification
A figure in a brief can be checked mechanically against the artefacts, and doing so found three stale claims in one pass. Load the measured quantities through surface_cov.load_targets, build the set of figures a brief is entitled to state, scan agents/*.md for numbers in the ranges those quantities occupy, and report every number matching no measured quantity. The false positives are informative on their own: the object graph reachability counts and the 1372 exported control methods minus 531 targets are real quantities from other artefacts, so the check wants a companion list naming what each unmatched figure counts. Run it after any change that moves the denominator, the exclusion groups or the declared syscall count.

## 2026-08-24T19:17:03+00:00 — describe
Tags: artefacts, digests, portability
A digest taken over a working copy is a digest of that machine's line endings. .gitattributes normalises this tree to LF, so a committed artefact is LF in the blob whatever platform wrote it, and git reports a CRLF working copy of it as unmodified. descriptions/generation.json recorded 74a740... for surface/drm-command-inventory.json, which is the sha256 of the CRLF working copy; the committed blob hashes to dd6853... The stale check therefore passed on the Windows workstation that recorded it and failed on every checkout of that commit, which is the Linux instance a campaign runs on. It was invisible because git showed the tree clean, and nothing else hashes a working copy. The rule: a check that hashes bytes on disk must say whether a mismatch is content or line endings, because the remedies are opposite and only one of them is a regeneration. regression_check stale now reports crlf when the file holds the recorded content with CRLF, and crlf-record when the file is LF and the digest was taken over a CRLF copy, which is the direction no checkout can reproduce.
