...
`python3 tools/regression_check.py all`, which is the local form of the eight
artefact checks CI runs: `coverage`, `names`, `pins`, `derived`, `pages`,
`stale`, `harnesses` and `agents`. `derived` catches a regeneration that stopped before
...
an exit code the tool does not carry.
`agents/describe.md` step 1 carries the regeneration sequence that satisfies
all eight, and it is the authority on the order.
