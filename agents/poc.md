...
PoCs. PoCs stop at "reliably triggers the vulnerability" and carry no
weaponization.
...
2. Track K: verify reproduction rate. Coordinate with the orchestrator for
   ...
   verify holds an flock on state/repro.lock for its whole run, so only one
   verification runs at a time and a second concurrent verify refuses.
   ...
   Exit code 2 means a rate was recorded on fewer counted runs than --runs
   requested, because the attempt cap fired. Check repro_runs_counted against
...
3. Track U: extract copies the crash input to artifacts/pocs/<id>/input
   ...
   harness in harnesses/TARGETS.md. Take it from there and do not
   reconstruct it. If it is missing, block the crash on the harness phase and
   ...
   mid-verify is recorded as void.
4. Track K profile check. Do this for every crash that reaches reliable or
   ...
   fresh user namespace. The threat model's attacker holds no such set. A
   container tenant has dropped capabilities, a seccomp filter and a device
   cgroup allowlist. Syzkaller therefore reaches paths the attacker cannot,
   and the gap runs in the direction that produces over-claims.
   ...
   |---|---|---|
   | `tenant-reachable` | The reproducer reproduces inside that container | The only outcome that supports the campaign's Track K claim that an unprivileged container tenant can reach this |
...
5. Write artifacts/pocs/<id>/README.md: build steps, run steps, expected
   ...
   attacker-controlled image, and the README states the exact privileges
   required.
6. If syz-manager never produced a reproducer (no repro.prog in the workdir,
   and older syzkaller builds name it repro.syz, which repro_ctl.py also
   accepts), say so in the README and mark the crash unreproducible. Do not
   hand-craft one from scratch.

## Panics during verification
Panics are expected, because a good kernel reproducer often takes the machine
down mid-run. repro_ctl.py persists progress before and after every run, so
the run that panicked is recovered when `verify` is re-invoked after the
reboot. It counts as a reproduction when the boot id changed AND the harvested
crash log (pstore/kdump/console) carries this crash's signature. A reboot
whose logs show a *different* crash is void, because the fuzzer panics this
box by design and any-reboot matching would inflate the rate that gates
disclosure. With no recoverable logs a boot-id change still counts, recorded
as a weaker evidence class in repro_progress. A verification process that
ended on the same boot (Ctrl-C, OOM kill, a repro that would not exec) is
recorded as void, and scores neither as a hit nor as a clean run. Re-run the
same command to resume. Use --restart only to discard the partial count
deliberately. Do not treat a crash that took the box down as a lost run, and
never restart the count silently, because that discards recorded
reproductions and understates the rate.
...
many were excluded. Timeouts are counted separately and never score as clean.
A hang-class crash title (hung task, watchdog, soft lockup, RCU stall,
deadlock) scores a timeout as a hit, and anything else scores it as void.
`--runs N` means N counted runs, so resuming with a smaller N never rewrites
an earlier, larger measurement, and it reports the accumulated one. If every
run comes back void the tool records no rate and exits 1. Investigate that,
and do not report a number.
...
`python3 tools/pipeline_ctl.py crash-list`, PoC README paths, and the
...
that is easy to miss. A **mistake** is about us: something that cost time,
