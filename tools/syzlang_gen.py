...
import checkout_meta  # noqa: E402  (path set above so the tool runs from anywhere)
...
# The banner's checkout field where --src holds no git checkout and --commit
# was not passed. The string names the reason, because the previous placeholder
# pointed the reader at version.mk, and version.mk records a driver release and
# never a commit.
NO_COMMIT = "unknown, --src is not a git checkout"
...
def recorded_commit(args):
    """The commit `emit` records, as (provenance value, banner text).

    Resolved from the `--src` checkout so the description set names the tree
    the way `surface/entry-points.json` and `tools/ioctl_map.json` name it.
    `--commit` overrides the resolution for a tree git cannot answer for, such
    as an unpacked release tarball.

    The two returned values differ where nothing resolves: the provenance
    value is None, which `descriptions/generation.json` carries as null, and
    the banner text is NO_COMMIT, which states the reason to a human reading
    the file header.
    """
    commit = args.commit or checkout_meta.checkout_commit(args.src)
    if not commit:
        logger.warning("no commit resolved from %s and no --commit passed, "
                       "the banner records %r", args.src, NO_COMMIT)
        return None, NO_COMMIT
    logger.info("recording commit %s for the checkout at %s", commit, args.src)
...
def cmd_emit(args):
    ...
    commit, banner_commit = recorded_commit(args)
    banner = FILE_HEADER % (version, banner_commit, args.header_name)
    ...
    manifest = {
        ...
        "generated_from": {
            ...
            # later reader has to reproduce the set from. Null where the
            # checkout is not a git tree: the banner carries the reason as
            # prose and this field carries a hash or nothing, so a check that
            # reads it never compares a sentence against a commit.
...
def build_parser():
    ...
    p.add_argument("--commit",
                   help="driver commit to record in the banner and in "
                        "generation.json (default: the short HEAD of --src)")
