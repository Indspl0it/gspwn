...
GPU counters originate in hardware registers.
The Resource Manager reads the registers, NVML exposes them, and `nvidia-smi`
and DCGM are clients of NVML. The Prometheus exporters read DCGM, not NVML.
CUPTI reaches the performance monitors on a separate path and requires
privileges NVML does not.
...
4. `dcgmi dmon -e 1002,1004` reports SM activity at 12% and tensor pipe activity
   ...
   resident.
5. The workload is small kernels launched back to back, each occupying a
   fraction of the device, so `utilization.gpu` stays at 99% while the SMs are
   mostly empty.
