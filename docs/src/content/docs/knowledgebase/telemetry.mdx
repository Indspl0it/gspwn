...
Every number a monitoring stack shows for a GPU originates in the same place
and travels the same path. Knowing the path explains why some figures are exact,
some are sampled, and one widely quoted figure means something other than its
name suggests.
...
NVML is the interface. `nvidia-smi` is a client of it, DCGM is a larger client
of it, and every exporter worth using is a client of one of those two. CUPTI
takes a separate path to the performance monitors and requires privileges that
NVML does not.
...
| Aggregate counts by class | `nvidia-smi -q -d PAGE_RETIREMENT,ECC` |
...
| `memory.total` | Capacity as the driver sees it, after ECC overhead |
...
The step that mattered was step 4. Steps 1 through 3 are available from
`nvidia-smi` alone and would have suggested a power problem as the whole
answer.
