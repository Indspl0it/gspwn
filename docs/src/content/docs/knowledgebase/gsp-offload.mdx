...
sequenceDiagram
  ...
  K->>G: kgspSetCmdQueueHead_HAL<br/>advances the queue head
...
| Queue head write | `kgspSetCmdQueueHead_HAL`, the register write that tells the GSP a message is waiting |
...
| Latency changed shape | Operations that were function calls became message round trips, so a sequence of small calls now pays one round trip each |
...
3. The kernel side builds an RPC message and `GspMsgQueueSendCommand` writes it
   into the command queue in device memory.
4. `kgspSetCmdQueueHead_HAL` advances the queue head register.
