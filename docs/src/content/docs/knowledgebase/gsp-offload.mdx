...
The Resource Manager used to run entirely in the host kernel. Since the R515
driver on Turing and later, most of it runs on a RISC-V processor inside the
GPU called the GSP, and the kernel module became a client of it. The open
kernel modules require this arrangement: they are the thin half of a driver
whose thick half is signed firmware.
...
The last row is the reason the open kernel modules exist at all, and it is
worth stating plainly: `open-gpu-kernel-modules` is open because the part that
NVIDIA does not publish moved to the other side of the queue.
