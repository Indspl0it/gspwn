...
independent: one space has the narrowest possible scope and the slowest
possible storage.
...
Local memory is the one that misleads. The name suggests on-chip storage next
to the thread; it is device memory. Local is where the compiler spills
registers, and where a thread-local array lands when it is indexed dynamically
so it cannot live in registers. A kernel that suddenly slows down after a small
edit has usually started spilling.
...
range as mostly-read or preferred-location. Both exist to avoid the fault
storm that a naive first-touch pattern produces.
