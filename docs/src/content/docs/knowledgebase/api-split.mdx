...
names the lower one, so the boundary between them appears in stack traces and
linker errors before it appears in the source a program was written from.
...
The runtime API creates the primary context lazily on first use, so the first
CUDA call in a process pays for context creation, module loading and, without
`nvidia-persistenced`, device initialisation.

## Kernel launch

`nvcc` rewrites the `<<<grid, block>>>` launch syntax, which no C++ compiler
accepts, into an ordinary function call before the host compiler sees it.
...
so a timing measurement taken without an intervening `cudaDeviceSynchronize`
measures the time to queue the command.
...
process starts at step 4, so the first call in a process costs context creation
and the second costs one allocation.
