...
its own ioctl numbering and its own view of memory. It exists to make one
allocation addressable from the CPU and from every GPU in the system, and to
move the pages so that whoever touches them finds them local.
...
The VA block is where the work happens. It tracks, for each page it covers,
...
Read duplication is the one that changes the shape of the problem. Under first
touch, two GPUs reading the same page in a loop migrate it back and forth. With
