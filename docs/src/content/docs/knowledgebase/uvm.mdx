...
| VA block | 2 MB of a range, `UVM_VA_BLOCK_BITS` 21 in `kernel-open/nvidia-uvm/uvm_va_block_types.h` | Per-page residency and mapping state for every processor |

A migration is a change to a VA block's per-page state plus the page table
...
in device memory and raises an interrupt, and the driver drains many entries at
...
first time pays this repeatedly at the start of its execution.
`cudaMemPrefetchAsync` avoids the repetition by performing steps 6 and 7 for
the whole range in one operation before the launch.
