...
CUDA device code passes through two compilers and two instruction sets. Which
one a binary carries determines whether it runs on a GPU released after it was
built.
...
The GPU executes SASS. Every path ends there, and the only variable is whether
`ptxas` ran when the binary was built or when it was loaded.
...
Forward compatibility is a property of PTX against newer hardware. It does not
make a newer toolkit's output loadable by an older driver, which is the failure
in the last row.
