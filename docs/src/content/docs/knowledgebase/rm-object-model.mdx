...
| Memory | Device | An allocation, host or device |
...
Most memory classes are parented by the device, several accept the subdevice as
well, and four are parented by the client root: `NV01_MEMORY_DEVICELESS`,
`NV_MEMORY_EXPORT`, `NV_MEMORY_FABRIC_IMPORT_V2` and
`NV_MEMORY_FABRIC_IMPORTED_REF`. Mapping an allocation into an address space is
a separate escape acting on two existing handles, and no class in the driver's
table names an address space as its parent. A channel accepts either the device
or a channel group as its parent, so a channel group is optional. The
authoritative list of legal parents per class is
`src/nvidia/src/kernel/rmapi/resource_list.h`, one `RS_ENTRY` record per class,
222 records at `610.57.04`.
