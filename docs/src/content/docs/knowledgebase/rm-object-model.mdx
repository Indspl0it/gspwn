...
Memory objects are parented by the device. Mapping an allocation into an
address space is a separate escape acting on two existing handles, and no class
in the driver's table names an address space as its parent. A channel accepts
either the device or a channel group as its parent, so a channel group is
optional. The authoritative list of legal parents per class is
