...
Destruction is recursive. Freeing the client frees everything under it, which
is what makes process exit clean up the GPU without cooperation from the
process.
...
Control commands are where most of the interface lives. Each is a number
selecting an operation on a class, with its own parameter structure. The
...
memory the process already has mapped, which is why submission is cheap and
setup is not.
