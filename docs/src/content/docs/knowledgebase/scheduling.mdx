...
submission path contains no system call: the pushbuffer and the doorbell are
memory the process has already mapped. Scheduling between queues is therefore
performed by hardware and firmware, with no host involvement per launch.
...
A context that has faulted is not recoverable. `cudaDeviceReset` destroys it
