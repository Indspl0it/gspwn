...
extended resource, and the components that make that happen are installed on
top. Everything below runs on one node.
...
The response to `Allocate` is where the two halves meet. The plugin returns
`NVIDIA_VISIBLE_DEVICES` set to the chosen UUIDs, and the container admission
path reads that variable when the container is created.
...
Steps 5 through 7 join orchestration to the container admission path. The
plugin decides which GPU; the toolkit makes it visible.
