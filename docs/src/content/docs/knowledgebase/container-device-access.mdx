...
flowchart TB
  ...
  CAP["Capability filter<br/>NVIDIA_DRIVER_CAPABILITIES<br/>default compute,utility"]
...
options, placing `/dev/nvidia-modeset` on it unless the driver option
`OPT_NO_MODESET` is set, and `nvc_driver_mount` (`src/nvc_mount.c:786`) then
filters that list with two tests: a device whose major is not 195 requires
`OPT_COMPUTE_LIBS`, and a device whose minor is 254 requires `OPT_DISPLAY`. The
node is therefore discovered by default and withheld from the container by the
second test. The per-GPU node is mounted separately by `nvc_device_mount`, with
no capability test at all.
...
The table above holds for the legacy hook alone. The CDI path reads no
capability value, and its device set is below.
...
not occur in its sources. The DRM nodes come from the Go runtime by two
separate routes. Under the legacy hook, `internal/modifier/graphics.go` builds
a DRM node discoverer only when the capability set contains `graphics` or
`display`, and matches the globs `/dev/dri/card*` and `/dev/dri/renderD*`
filtered to the PCI bus IDs of the visible GPUs. Under the CDI path,
`GetDeviceNodesByBusID` (`internal/info/drm/drm_devices.go`) globs
...
the per-GPU device edit with no capability test. `internal/edits/device.go`
...
cgroup v1 and `src/cgroup.c` for v2). On this path the `m` permission is never
granted, so a container cannot create further nodes on those majors. The CDI
path grants `rwm` on every device edit, so the two mechanisms differ on `m` as
well as on which nodes they inject.
