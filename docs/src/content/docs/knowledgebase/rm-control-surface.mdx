...
Four counts describe how far that gets, over the same 531 commands.
...
| Whose owning class has an `RS_ENTRY` row or a subclass that does | 531 | A command compiled into a base class reaches an object allocated as any class deriving from it, so those 15 borrow a subclass's chain |
| With a chain an unprivileged process can build | 529 | The 2 absent are on `MmuFaultBuffer` and `NvDispApi`, whose every external class requires allocation privilege |

The chain lengths concentrate at the short end. 424 of the 529 are reachable
