...
NVOC emits one entry per exported control method.
...
`rmresControl_Prologue_IMPL` in
`src/nvidia/src/kernel/rmapi/resource.c` issues `NV_RM_RPC_CONTROL` itself and
then returns `NV_WARN_NOTHING_TO_DO`, which tells the caller the work is
already done and the local handler is to be skipped. The implementation lives
in signed firmware on the GSP.
...
`rm-control-rank.json` carries the same decomposition on each record, in its
`no_chain_reason` field, tabulated on
...
Control is the largest of the seven ioctl families a tenant can reach. Its 531
targetable commands sit in a denominator of 852, beside escape 32, uvm 39,
uvm_tools 7, alloc 155, modeset 64 and drm 24. The 841 exported control
commands outside that 531 are the unmodelled remainder of this one family.
...
`tools/ctrl_rank.py` ranks all 531 and writes `surface/rm-control-rank.json`,
the artefact behind
[Control commands](/gspwn/reference/surface/control-commands/). The ordering is
consumed as a prefix: the number of commands modelled is configurable, and a
lower setting keeps the head of the ranking and drops the tail. The ordering
therefore decides which commands are reached at all when the full 531 are not.

Three measurements separate the commands.
...
[Control commands](/gspwn/reference/surface/control-commands/) lists all 531 in
rank order with their components. The head is entirely `RmClientResource`
because `depth` outweighs the other two components and a client-level command
costs one allocation. The tail is the 17 with no chain, and the last three
records are `Memory`, `MmuFaultBuffer` and `ProfilerBase`.
