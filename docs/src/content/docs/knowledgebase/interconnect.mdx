...
BAR1 is the constraint behind several failures. It is sized at boot, and where
it is smaller than device memory the host can only see a window at a time. This
is what resizable BAR addresses: with it enabled and supported by the platform
firmware, BAR1 covers the whole of device memory and host mappings need no
window movement.
...
A GPU running at a lower link width than the slot supports is usually a
platform or seating problem, and `nvidia-smi -q -d PCI` names both the current
and the maximum.
...
them. With it, every GPU reaches every other at full link bandwidth, which is
what makes all-to-all collectives uniform on an 8-GPU baseboard.
...
Its absence is the usual reason a working NVLink setup still moves data through
the CPU when the network is involved.
...
Tracing the third route, which is what an application sees when peer access
silently fails:
...
The transfer crossed PCIe twice, waited once in the middle, and consumed host
memory bandwidth throughout. Enabling peer access with
