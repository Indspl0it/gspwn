...
| `loop.campaign_hours` | How long each campaign runs before its deadline timer stops and disables the units | number | `> 0`, and not above `loop.max_total_run_hours` | `1000` | `campaign_ctl.cmd_install_k`, `campaign_ctl.cmd_install_u` |
| `loop.stop_on_plateau` | Whether a `plateaued` coverage verdict ends the loop | boolean | `true` or `false`, unquoted. A quoted string is truthy and silently keeps the default behaviour | `false` | `pipeline_state.loop_decision` |
...
denominator is counted at 852, and the completion rule over it invents no
...
| `coverage.horizon_hours` | How far ahead the fitted curve is extrapolated. Matching `loop.campaign_hours` scopes the verdict to exactly one further campaign | number | `> 0` | `1000` | `coverage_ctl.plateau_verdict` |
