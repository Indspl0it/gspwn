...
description: The impact record schema, the derived CWE mapping, the four support gaps that block a severity, and the chain the report writes to argue one.
---
...
Without the second, a severity is an assertion.

## The two halves

A severity rests on two answers, and two different phases produce them.
...
A controlled write no unprivileged process can trigger and a null dereference
any tenant can reach are both real findings, and neither is critical. The
`report` phase joins them.
...
through `pipeline_ctl.py impact-set`. Four carry a judgement, seven are
transcribed from the sanitizer report, two hold free-text analysis and four
record provenance. The eighteenth is `cwe`, which is derived and set only to
override the derivation.

### Judgement fields

Each judgement field takes its value from a closed vocabulary.
...
| `overwrite_target` | `function-pointer`, `length-or-size`, `refcount`, `index-or-offset`, `list-pointer`, `flags-or-state`, `data-buffer`, `unknown`, `not-applicable` | Which field the corruption overwrites. An overwritten function pointer and an overwritten flags byte are the same memory-safety bug and different vulnerabilities |
| `attacker_control` | `allocation-timing`, `allocation-size`, `written-data`, `written-offset`, `freed-pointer`, `object-lifetime`, `call-ordering`, `none`, `unknown` | What the attacker influences. A list, holding any number of these values. A consequence above denial of service argued from an attacker who influences nothing is challenged first |

The other three are single-valued. `impact-set` refuses a value outside the
vocabulary, and refuses any entry of `attacker_control` outside it.

### Transcription fields

These fields are copied out of the sanitizer report and involve no judgement.

| Field | Source |
|---|---|
| `access_type` | The read, write or free the sanitizer reported. KASAN reports it verbatim |
| `access_size` | Bytes, from the sanitizer report. A positive integer, or absent |
...
These fields answer a question the vocabularies cannot hold.

- `reclaim_path`, for a use-after-free: whether the attacker can get the freed
  allocation back with data they chose, and by what mechanism.
- `race_window`, for a race: what has to interleave.

Both are free text and both may be left empty.

### Provenance fields

These fields record where the analysis came from and how far it was checked.
...
applies. An override has to match `CWE-` followed by digits, and `impact-set`
refuses anything else. `bug_class` is a closed vocabulary, so the two cannot
disagree by drift.

`cwe_of()` returns an empty string in two cases: the crash carries no research
record, or its `bug_class` is `other` and no override was written. The rollup
groups those records under `CWE-unmapped`. An empty CWE is honest and a guessed
one is not.
...
An invented impact narrative is the most costly outcome, so only an unexplained
`undetermined` is reported as a problem.

## Support gaps

`impact_support_gap()` returns the first of four conditions that applies, in
...
| 2. Claims a primitive with no evidence | `primitive` is one of the six that assert something about code, `info-leak`, `uncontrolled-write`, `controlled-write`, `controlled-free`, `refcount-imbalance` or `type-confusion`, and `evidence` is empty | `That is a claim about code, so it needs the file:line it rests on before a report can put a severity on it` |
| 3. Consequence outruns its mechanism | `consequence` is `privilege-escalation` or `container-escape`, and `primitive` is `none` or `undetermined` | `The conclusion outruns the mechanism: name what the fault actually gives an attacker, or lower the consequence` |
| 4. Consequence rests on an attacker who controls nothing | `consequence` is `privilege-escalation` or `container-escape`, and `attacker_control` is empty or holds only `none` and `unknown` | `An outcome above denial of service needs the attacker to influence something. With no influence, the defensible answer is dos-only` |
...
The `report` phase writes the argument explicitly, as six links in order. Each
link names the field it rests on, and the phase that produced that field.

```mermaid
flowchart LR
  W["weakness<br/>CWE-416"] --> P["primitive<br/>controlled-write"]
  P --> L["what it overwrites<br/>function-pointer,<br/>in a reclaimable object"]
  L --> A["what the attacker controls<br/>allocation-timing,<br/>written-data"]
  A --> R["reachability<br/>tenant-reachable"]
  R --> C["consequence<br/>privilege-escalation"]

  BC["finding.bug_class<br/>rca"] -.-> W
  IP["impact.primitive<br/>rca"] -.-> P
  OT["impact.overwrite_target<br/>impact.corrupted_object<br/>rca"] -.-> L
  AC["impact.attacker_control<br/>rca"] -.-> A
  PC["profile check<br/>poc"] -.-> R
  IC["impact.consequence<br/>rca"] -.-> C
```

Solid edges are the argument. Dashed edges are the field each link is read
from. Five of the six come from `rca`, and reachability comes from `poc`,
which is why no single phase can assert a severity on its own.
...
Four checks stop an impact record from going missing, or from carrying a
conclusion it does not support, without anyone noticing.
